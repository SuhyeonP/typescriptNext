...
} from '../../exporthing/mineAction';
import Form from '../../component/game/Form';
import Table from '../../component/game/Table';
...
interface Context {
    tableData: Codes[][],
    halted: boolean,
    dispatch: Dispatch<ReducerActions>,//explain:실제 액션을 제너릭으로 넣어줌
}
export const TableContext = createContext<Context>({
    tableData: [],
    halted: true,
    dispatch: () => { },
})//explain:칸칸에 보낼 데이터를 createContext를 통해서 보낼것이다.
//컴포넌트간의 공유할 데이터를 createContext로 이용한다. 빈배열있으면 never혹은 any가되면,오류날수도 있어서
//막기위해 제너릭으로 타이핑을 해줌
...
const plantMine = (row: number, cell: number, mine: number): Codes[][] => {
    ...
    const shuffle = [];//섞기위해서 빈배열에 채워넣어질것임
        ...
        <>
            ...
            <section className="mine-section">
                <TableContext.Provider value={value}>
                    <div className="mine-result">{result}</div>
                    <Form />
                    <div>시간 : {timer}</div>
                    <Table />
